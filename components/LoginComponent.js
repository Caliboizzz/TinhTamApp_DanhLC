...
class Login extends Component {
    ...
    return (
      <View style={{ justifyContent: 'center', margin: 20 }}>
        ...
        <View style={{ marginTop: 20 }}>
          <Button title='Login' buttonStyle={{ backgroundColor: '#00d9a5',borderRadius:25 }} onPress={() => this.handleLogin()} />
        </View>
      </View>
    );
  }
  ...
  handleLogin() {
    ...
    get(child(dbRef, 'accountsuser/' + this.state.username)).then((snapshot) => {
      if (snapshot.exists()) {
